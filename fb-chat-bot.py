...
class ChatBot(Client):
    ...
    def onReactionRemoved(self, mid=None, author_id=None, thread_id=None, thread_type=ThreadType.USER, **kwargs):
        ...
        self.send(Message(text=reply), thread_id=thread_id,
                  thread_type=thread_type)
    def onPeopleAdded(mid=None, added_ids=None, author_id=None, thread_id=thread_id, ts=None, msg=None):
        reply = "{} added: {} in {}".format(author_id, ", ".join(added_ids), thread_id)
        self.send(Message(text=reply), thread_id=thread_id,
                  thread_type=thread_type)

    def onPersonRemoved(mid=None, removed_id=None, author_id=None, thread_id=thread_id, ts=None, msg=None):
